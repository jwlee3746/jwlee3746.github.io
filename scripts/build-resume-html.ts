...
import { renderResume } from '../ui/resume/layout.ts';
...
export async function buildResume(root = projectRoot, check = false): Promise<string> {
  ...
  const output = resolve(root, '_site/resume/index.html');
  if (check) {
    if (await readFile(output, 'utf8') !== html) {
      throw new Error('_site/resume/index.html이 원본과 다릅니다. npm run build:resume을 실행하세요.');
    }
  } else {
    await mkdir(resolve(root, '_site/resume'), { recursive: true });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (args.some(arg => arg !== '--check')) {
    ...
    try {
      ...
      console.log(args.includes('--check') ? '이력서 HTML 동기화 확인 완료' : '생성 완료: _site/resume/index.html');
    } catch (error) {
