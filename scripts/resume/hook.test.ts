...
async function repository(t: { after: (fn: () => Promise<void>) => void }) {
  ...
  await writeFile(join(root, 'resume/index.html'), 'original html');
...
test('hook stages both generated artifacts when a project is staged', async t => {
  ...
  assert.equal(git('show', ':resume/index.html'), 'generated');
...
test('hook rejects partially staged data instead of publishing unstaged text', async t => {
  ...
  assert.equal(git('show', ':resume/index.html'), 'original html');
});
...
test('hook rejects untracked projects when rebuilding', async t => {
  ...
  assert.equal(await readFile(join(root, 'resume/index.html'), 'utf8'), 'original html');
});
