...
INPUTS='^(data/resume/.*\.json|ui/(resume|shared)/.*|public/favicon\.svg|scripts/(build-resume-(html|pdf)\.ts|build-resume-pdf\.sh|resume/.*)|jaewon-lee-resume\.pdf|package(-lock)?\.json|tsconfig\.json|\.nvmrc)$'
