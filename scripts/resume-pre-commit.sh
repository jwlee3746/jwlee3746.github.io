...
INPUTS='^(data/(site/resume\.json|profile/resume\.json|portfolio/(resume\.json|experience\.json|education\.json|projects/.*\.json))|theme/resume/.*|templates/(resume|shared)/.*|public/favicon\.svg|scripts/(build-resume-(html|pdf)\.ts|build-resume-pdf\.sh|resume/.*)|jaewon-lee-resume\.pdf|package(-lock)?\.json|tsconfig\.json|\.nvmrc)$'
