# 부록: AIC runtime의 7단계 변화

[본문: 프롬프트 평가에서 에이전트 평가로](aic-evolution-and-evaluator.md)

이 부록의 7단계는 **AIC runtime의 구현 이력**을 구분한다. 본문의 6개 phase는 **평가 문제와 평가 방식의 변화**를 구분하므로 번호가 일대일로 대응하지 않는다. 기존 분석의 다이어그램 14개와 근거는 그대로 보존했다.
