...
export function renderPanel(posts: readonly Post[], toc?: string, { showTags = true, showRecent = true } = {}): string {
  ...
  return `<aside class="site-panel" aria-label="글 바로가기">
    ${showRecent ? `<section><h2>최근 글</h2><ul class="site-panel-list">${recent.map(post => `<li><a href="${e(post.url)}" title="${e(post.data.title)}">${e(post.data.title)}</a></li>`).join('')}</ul></section>` : ''}
