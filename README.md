...
이전한 글 목록은 `/blog/posts/`, 가져온 글은 기존 `/blog/Algorithm/1208/` URL로 생성합니다.
`/blog/`는 전체 글 목록으로 이동합니다. 아직 이전하지 않은 글은 이 사이트에서 제공하지 않습니다.
...
현재 영역의 하위 메뉴가 펼쳐지고 화살표 버튼으로 다른 영역도 열 수 있습니다.
...
- 분류별 목록은 `/blog/categories/<분류 이름>/`, 태그는 `/blog/tags/`에 생성됩니다.
- 전체 글 목록에서 제목·요약·분류·태그를 검색합니다. 홈 검색도 이 목록으로 연결됩니다.
- 검색은 브라우저에서 동작하며 본문 전체 검색은 포함하지 않습니다. JavaScript가 없어도 전체 목록과 메뉴 링크는 사용할 수 있습니다.
