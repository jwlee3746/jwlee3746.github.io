...
외부 블로그가 참조하는 `assets/avatar.jpg`는 호환용 복사본이며 원본은 `images/avatar.jpg`입니다.
프로필 교체 시 두 파일을 함께 갱신합니다. `favicon.svg`는 사이트 공통 진입 경로로 루트에 유지합니다.

## 로컬 미리보기

저장소 루트에서 실행한 뒤 <http://localhost:8000/>을 엽니다.
리소스 경로가 `/theme/`·`/images/` 등 절대경로이므로 HTML 파일을 직접 열지 말고 HTTP 서버를 사용합니다.
