...
date: 2024-05-24
permalink: /posts/mle-interview-math-statistics/
legacyUrl: "/blog/Machine Learning/MLE기술면접대비질문/"
tags: []
toc: true
---
...
양성판정 정확도는 $P(\text{양성}\|A) = 0.9$이고, 구하고자 하는 것은 $P(A\|\text{양성})$이다.  
질병이 전세계 1% 확률로 발현되고 검사의 음성 판정 정확도($P(\text{음성} \| \neg A)$)가 똑같이 90% 라면,

$$\begin{aligned}P(A|\text{양성}) &= \frac{P(\text{양성}|A) \cdot P(A)}{P(\text{양성})} \\ &= \frac{P(\text{양성}|A) \cdot P(A)}{P(\text{양성} | A) \cdot P(A) + P(\text{양성} | \neg A) \cdot P(\neg A)} \\ &= \frac{0.9 \times 0.01}{0.108} \approx 0.0833\end{aligned}$$
...
<iframe src="https://www.youtube-nocookie.com/embed/HZGCoVF3YvM" title="Bayes 정리 참고 영상" width="560" height="315" style="width: 100%; height: auto; aspect-ratio: 16 / 9; border: 0" loading="lazy" allowfullscreen></iframe>
