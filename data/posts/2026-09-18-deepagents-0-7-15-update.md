...
![deepagents 0.7.9부터 0.7.15까지 평가, 파일 도구, subagent 문맥, 긴 실행 복구로 묶은 릴리스 지도](/data/images/posts/deepagents-0-7-15-update/release-map.svg)

표의 변경을 기능 영역으로 다시 묶으면, 새 capability 하나보다 실행의 경계를 분명하게 만든 흐름이 보인다. 0.7.12의 conversation fork가 가운데에 있고, 앞뒤 릴리스는 평가 누락·도구 실패·긴 실행 복구처럼 운영 중 드러나는 모호함을 줄인다.
...
![isolated는 위임 설명만 전달하고 fork는 부모 대화와 위임 설명을 함께 복사하는 subagent 문맥 비교](/data/images/posts/deepagents-0-7-15-update/subagent-context-modes.svg)
