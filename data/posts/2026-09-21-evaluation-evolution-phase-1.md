---
title: '[평가의 진화 1] 고정된 프롬프트에서 plan을 평가하다'
excerpt: 고정된 입력과 정답으로 모델의 plan을 평가하는 구조를 살펴보고, 판단 정확도와 실제 작업 성공 사이의 경계를 정리한다.
date: '2026-09-21'
category: Evaluation
tags:
- Agent
...
**초기 평가의 중심 질문은 “이 문맥에서 모델이 올바른 다음 plan을 생성하는가?”였다.** 입력과 정답을 고정해 모델의 판단을 비교할 수 있었지만, 그 판단을 실제로 실행한 뒤 사용자의 목표까지 달성했는지는 별도의 문제였다.

- **시리즈의 출발점:** prompt 기반 planning 평가 → 판정 고도화 → agent harness → 계약 기반 fixture replay → stateful simulator → 하이브리드 평가.
- **이번 글의 범위:** 고정 prompt 평가의 입력 구성, 추론, 결과 기록과 채점의 경계.
...
| 입력 상태 | 대화 이력과 참조 대상을 prompt로 표현하기 위한 입력 상태 |
| 입력 구성 | 상태와 schema 정보를 모델 입력으로 변환 |
| Model | 입력을 받아 agent·함수·파라미터·plan 등을 생성 |
| Ground truth | 해당 입력에서 기대하는 출력 |
| Grader | 실제 출력과 기대값을 비교해 정오답과 오류 정보를 생성 |

- **평가 준비:** TC에서 모델 입력과 기대 출력을 구성.
- **추론과 기록:** 준비된 입력으로 모델을 호출하고 예측 결과를 후속 채점에 전달.
- **공유된 표현:** 평가와 실제 planning에서 문맥·함수 정보를 같은 방식으로 표현해 입력 구성의 차이를 줄임.
- **구분할 상태:** TC에서 구성한 입력 상태가 있다는 사실만으로 실제 tool 호출이 환경을 바꾸고 그 결과를 다시 관찰하는 평가가 되는 것은 아님.
...
## 2. 평가 파이프라인: TC에서 추론 기록까지

![고정 prompt 기반 평가의 블록 다이어그램](/data/images/posts/evaluation-evolution-phase-1/evaluation-pipeline.svg)

*그림 1. TC를 입력·정답 쌍으로 바꾸고 prediction을 기록한 뒤 채점하는 구조. 실제 업무 도구 실행과 최종 환경 상태 검증은 이 경로의 바깥에 있다.*

1. **예제 구성:** TC의 발화·대화 문맥과 schema로 입력 상태와 기대 출력을 준비.
2. **입력 생성:** 문맥과 함수 정보를 prompt로 구성하고, 비교할 정답을 함께 준비.
...
- **분리의 장점:** 동일한 추론 기록을 두고 오답을 분석하거나 채점 결과를 다시 살펴볼 수 있음.
...
- **입력 전처리:** 입력 표현을 모델 호출 형식에 맞추고, 실제 추론에 쓴 입력을 결과에 반영.
- **기록할 정보:** 입력, 정답, 평가 대상의 종류와 모델의 예측을 함께 보존.
- **진행 중 지표:** 앞뒤 공백을 제거한 response와 target의 문자열 일치 여부로 정확도를 표시.
- **사후 판정:** 채점 단계에서는 문자열 비교에 더해 plan을 파싱하고 함수·파라미터를 재귀적으로 비교.
...
여러 줄의 plan을 분할하는 평가 경로는 다음과 같이 동작한다.
...
- **예외:** 살펴본 방식에서는 한 줄 target과 평가 모드가 아닌 경우에는 분할하지 않음. 자연어 응답·대상 연결·요약처럼 plan 줄 단위 분할이 맞지 않는 출력도 제외.
...
- **판정 범위의 예:** 생성된 plan에 기대하는 agent가 포함됐는지만 검사하는 평가도 있었음. 그 통과를 모든 인자·순서·실행 효과의 검증으로 해석할 수 없음.
...
- **장기 방향:** 고정 fixture 평가와, state·tool output을 실제 action에 따라 제공하는 simulator 평가를 함께 운영. 이 시리즈에서 다룰 **향후 설계 방향**이며 구현 완료를 뜻하지 않음.
...
- **서술 범위:** 초기 prompt 기반 평가의 기본 단위를 재구성한 설명. 모든 기능이 처음부터 함께 존재했다거나 모든 평가 경로가 이 구조였다는 뜻은 아님.
