...
새로 작성하거나 수정하는 도표는 본문과 같은 `--font-body` 웹폰트를 사용합니다.
본문 설명은 `1rem`, 보조 설명은 `.875rem`, 제목은 `1.125rem` 이내이며 강조는 `--weight-semibold`를 사용합니다.
`system-ui`나 도표용의 큰 글자를 별도로 지정하지 않습니다.

저장소에서 직접 만든 SVG는 루트에 `data-article-diagram="true" class="article-diagram-art"`를 지정하고
Markdown의 일반 이미지 문법으로 연결합니다. `ui/posts/diagrams.ts`가 글 본문에 inline SVG로 삽입해
웹폰트를 공유하고, `ui/posts/article.css`가 글꼴과 크기를 통일합니다. SVG 내부 스타일은
`.article-diagram-art` 아래로 한정하여 본문에 영향을 주지 않게 합니다.
출처가 외부인 이미지에는 이 표시를 붙이지 않습니다.

SVG의 `width`·`height`는 `viewBox`와 같은 좌표 단위를 사용합니다. 본문에서 확대·축소하지 않고,
좁은 화면에서는 키보드·터치로 도표 영역만 가로 스크롤합니다. 데스크톱 1440px와 모바일 390px에서
실제 글꼴·크기, 텍스트 겹침, 도표 끝까지의 접근을 확인합니다.
