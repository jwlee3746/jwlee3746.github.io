...
| `resume/index.html` | A4 한 페이지 인쇄용 이력서 (`@page size: A4`) |
| `resume/jaewon-lee-resume.pdf` | 생성 산출물 |
| `resume/build-pdf.sh` | PDF 생성. 한 페이지를 벗어나거나 렌더가 잘못되면 실패로 끝난다 |
...
- 이력서는 **A4 한 페이지** 안에서 프로젝트 근거와 가독성을 우선한다. 빌드 스크립트가 페이지 수를 검증한다.
