...
- 글에 새로 만들거나 수정하는 도표는 **본문과 같은 글꼴과 크기 체계**를 사용한다. 본문 설명은 `--font-body`·`1rem`, 보조 설명은 `.875rem`, 도표 제목은 `1.125rem` 이내로 맞춘다. Inline SVG로 본문 웹폰트를 공유하고, 모바일에서는 전체 축소 미리보기와 클릭 확대를 함께 제공한다. 확대창의 100% 보기에서 위 글자 크기를 유지한다. 상세 구현은 `data/README.md`를 따른다.
...
메인·이력서 데이터 모두 `2020 — 2023`으로 채웠다. 근거는 노션 아카이브에 남은
