...
| `pages/portfolio/` | 메인 문서·섹션 조립 |
| `pages/404.11ty.ts`, `data/site/error.json` | 오류 페이지 조립·문구 |
...
- 이력서 원본 데이터·`pages/resume/`·`pages/shared/`·`theme/resume/`·빌드 코드를 고치면 `npm run build:resume:pdf`로 HTML과 PDF를 함께 갱신한다. `npm run typecheck`, `npm test`, `npm run check:resume`도 통과해야 한다.
